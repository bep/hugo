...
func (pco *pageContentOutput) initRenderHooks() error {
	...
	pco.renderHooks.init.Do(func() {
		...
		pco.renderHooks.getRenderer = func(tp hooks.RendererType, id any) any {
			...
			consider := func(candidate *tplimplv2.TemplInfo) bool {
				...
				if ignoreInternal && candidate.SubCategory == tplimplv2.SubCategoryInternal {
			...
			getHookTemplate := func() (*tplimplv2.TemplInfo, bool) {
				q := tplimplv2.TemplateQuery{
					LockType: doctree.LockTypeRead,
					Dir:      dir,
					Category: tplimplv2.CategoryMarkup,
					Desc:     layoutDescriptor,
					Consider: consider,
				}

				// TODO1 check Dir vs bundle etc.
...
func executeToString(ctx context.Context, h *tplimplv2.TemplateStore, templ *tplimplv2.TemplInfo, data any) (string, error) {
	b := bp.GetBuffer()
