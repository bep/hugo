		...
		for {
			...
			if err := func() error {
				...
				if !deferred.Executed {
					tmpl := s.Deps.Tmpl() // TODO1 get rid of me?
...
func (h *HugoSites) printUnusedTemplatesOnce() error {
	h.printUnusedTemplatesInit.Do(func() {
		...
		if conf.PrintUnusedTemplates {
			unusedTemplates := h.Tmpl().Unused()
			for k, unusedTemplate := range unusedTemplates {
				if unusedTemplate.Fi != nil {
					h.Log.Warnf("Template %s is unused, source %q", k, unusedTemplate.Fi.Meta().Filename)
				} else {
					h.Log.Warnf("Template %s is unused", k)
				}
			}
		}
	})
	...
	handleChange := func(pathInfo *paths.Path, delete, isDir bool) {
		...
		case files.ComponentFolderLayouts:
			...
			if tmplAdded {
				...
				id := h.Tmpl().GetIdentity(pathInfo.Path())
				if id != nil {
