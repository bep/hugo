...
func (p *pageContentMap) contentToRender(key string, source []byte, renderedShortcodes map[string]string) []byte {
	defer herrors.Recover(func(r any) []any {
		fmt.Println("contentToRender for source\n", key, len(source), "=>\n", string(source))
		return nil
	})
	if len(p.items) == 0 {
		return nil
...
func (c *cachedContent) contentRendered(cp *pageContentOutput) (contentTableOfContents, error) {
	...
	v, err := c.getOrCreate(key, &cp.version, func(ctx context.Context) (any, error) {
		...
		contentToRender := c.pageContentMap.contentToRender(key, source, contentPlaceholders)
