...
func newBranchMap(createBranchNode func(elem ...string) (*contentNode, error)) *branchMap {
	return &branchMap{
...
func newContentBranchNode(n *contentNode) *contentBranchNode {
	return &contentBranchNode{
		...
		refs:          make(map[any]ordinalWeight),
	}
}
...
// GetBranchOrLeaf can be used if you don't known if the key represents a leaf or branch.
// It will first try to reseolve it as a branch then leaf.
// It returns nil if not found.
func (m *branchMap) GetBranchOrLeaf(key string) *contentNode {
	if branch := m.GetBranch(key); branch != nil {
		return branch.n
	}

	_, branch := m.GetBranchContainer(key)
	if branch == nil {
		return nil
	}
...
func (m *branchMap) InsertBranch(n *contentNode) *contentBranchNode {
	_, b, err := m.InsertRootAndBranch(n)
	if err != nil {
		panic(err)
	}
	return b
...
func (m *branchMap) InsertRootAndBranch(n *contentNode) (root *contentBranchNode, branch *contentBranchNode, err error) {
	...
	if strings.Count(n.key, "/") > 1 {
		...
		if !found || s == "" {
			nkey := n.KeyParts()
			rkey := nkey[:1]
			n, err := m.createBranchNode(rkey...)
			if err != nil {
				return nil, nil, err
			}
			root = newContentBranchNode(n)
...
func (m *branchMap) GetLeaf(s string) *contentNode {
	_, branch := m.GetBranchContainer(s)
	if branch != nil {
...
// GetBranch returns the branch with key s.
func (m *branchMap) GetBranch(key string) *contentBranchNode {
...
// GetBranchContainer gets the branch closest up from s.
func (m *branchMap) GetBranchContainer(s string) (string, *contentBranchNode) {
	k, v, found := m.branches.LongestPrefix(paths.Dir(s))
	if !found {
		return "", nil
	}
...
func (m *branchMap) Walk(q branchMapQuery) error {
	...
	handleBranchPage := func(depth depthType, s string, v any) bool {
		...
		if depth <= depthBranch {
			...
			if q.Branch.Resource != nil {
				bn.resources.nodes.Walk(func(s string, v any) bool {
					n := v.(*contentNode)
		...
		if q.Leaf.Page != nil || q.Leaf.Resource != nil {
			bn.pages.nodes.Walk(func(s string, v any) bool {
				n := v.(*contentNode)
				...
				if q.Leaf.Resource != nil {
					...
					bn.pageResources.nodes.WalkPrefix(s+"/", func(s string, v any) bool {
	...
	if !q.Branch.Key.IsZero() {
		...
		if q.Branch.Key.IsPrefix() {
			...
			m.branches.WalkPrefix(
				q.Branch.Key.Value, func(s string, v any) bool {
		...
		// Exact match.
		section := m.GetBranch(q.Branch.Key.Value)
		if section != nil {
	...
	_, section := m.GetBranchContainer(q.Leaf.Key.Value)
	if section == nil {
		return nil
	}
	...
	if q.Leaf.Resource != nil {
		section.pageResources.nodes.WalkPrefix(q.Leaf.Key.Value+"/", func(s string, v any) bool {
...
func (m *branchMap) WalkBranches(cb func(s string, n *contentBranchNode) bool) {
	m.branches.Walk(func(s string, v any) bool {
...
func (m *branchMap) WalkBranchesPrefix(prefix string, cb func(s string, n *contentBranchNode) bool) {
	m.branches.WalkPrefix(prefix, func(s string, v any) bool {
		return cb(s, v.(*contentBranchNode))
	})
}
...
func (m *branchMap) newNodeProviderPage(s string, n *contentNode, owner, branch *contentBranchNode, deep bool) contentNodeProvider {
	...
	if owner == nil {
		if s != "" {
			_, owner = m.GetBranchContainer(s)
		}
	}
...
func (m *branchMap) debug(prefix string, w io.Writer) {
	...
	m.WalkBranches(func(s string, n *contentBranchNode) bool {
		...
		n.pageResources.Walk(func(s string, n *contentNode) bool {
			fmt.Fprintf(w, "\tBranch Resource: %q\n", s)
			return false
		})
		n.pageResources.Walk(func(s string, n *contentNode) bool {
			fmt.Fprintf(w, "\tLeaf Resource: %q\n", s)
			return false
		})
		return false
	})
...
func (m *contentBranchNode) newResource(n *contentNode, owner *pageState) (resource.Resource, error) {
	...
	ownerPath := owner.File().FileInfo().Meta().PathInfo
	meta := fim.Meta()
...
func (t contentBranchNodeTree) Walk(cb ...contentTreeNodeCallback) {
	...
	t.nodes.Walk(func(s string, v any) bool {
...
func (t contentBranchNodeTree) WalkPrefix(prefix string, cb ...contentTreeNodeCallback) {
	cbs := newcontentTreeNodeCallbackChain(cb...)
	t.nodes.WalkPrefix(prefix, func(s string, v any) bool {
		return cbs(s, v.(*contentNode))
	})
}
...
func (t *defaultNodeTree) Delete(s string) (any, bool) {
...
func (t *defaultNodeTree) Insert(s string, v any) (any, bool) {
	switch n := v.(type) {
	case *contentNode:
		n.key = s
...
type nodeTree interface {
	Delete(s string) (any, bool)
	DeletePrefix(s string) int

	// Update ops.
	Insert(s string, v any) (any, bool)
	Len() int

	LongestPrefix(s string) (string, any, bool)
	// Read ops
	Walk(fn radix.WalkFn)
	WalkPrefix(prefix string, fn radix.WalkFn)
	Get(s string) (any, bool)
}
...
func (t *nodeTreeUpdateTracer) Delete(s string) (any, bool) {
...
func (t *nodeTreeUpdateTracer) Insert(s string, v any) (any, bool) {
	var typeInfo string
	switch n := v.(type) {
	case *contentNode:
		typeInfo = "n"
