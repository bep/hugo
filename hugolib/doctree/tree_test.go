...
var (
	shiftFunc = func(n *testValue, dimension []int) (*testValue, bool) {
		if n.noCopy {
	...
	insertFunc = func(old, new *testValue) (*testValue, bool) {
		return new, true
	}
)
...
func TestTreeInsert(t *testing.T) {
	...
	tree := doctree.New(
		doctree.Config[GetLangProvider]{
			Dimensions: []int{0, 0},
			Shift: func(n GetLangProvider, dimension []int) (GetLangProvider, bool) {
				return n, true
			},
			Insert: func(GetLangProvider, GetLangProvider) (GetLangProvider, bool) {
				return &getLangProviderSlice{}, true
			},
		},
	)
	...
	c.Assert(v, qt.DeepEquals, &getLangProviderSlice{})

	// This test isn't very realistic, but good enough for now.
	tree1 := tree.Increment(0)
	c.Assert(tree1.Get("/a/b"), qt.DeepEquals, &getLangProviderSlice{})
}
...
func TestTreeData(t *testing.T) {
	...
	tree := doctree.New(
		doctree.Config[*testValue]{
			Dimensions: []int{0, 0},
			Shift:      func(n *testValue, dimension []int) (*testValue, bool) { return n, true },
		},
	)
...
type getLangProviderSlice []string

func (g getLangProviderSlice) getLang() int {
	return len(g)
}
