...
func newHugoSites(cfg deps.DepsCfg, d *deps.Deps, pageTrees *pageTrees, sites []*Site) (*HugoSites, error) {
	...
	for i, s := range sites {
		...
		if i == 0 {
			templateStore, err := tplimplv2.NewStore(
				tplimplv2.StoreOptions{
...
func (s *Site) renderAndWritePage(statCounter *uint64, name string, targetPath string, p *pageState, d any, templ *tplimplv2.TemplInfo) error {
...
type hookRendererTemplate struct {
	templateHandler *tplimplv2.TemplateStore
...
func (s *Site) renderForTemplate(ctx context.Context, name, outputFormat string, d any, w io.Writer, templ *tplimplv2.TemplInfo) (err error) {
	if templ == nil {
